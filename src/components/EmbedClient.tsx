...
function EmbedClient({ownerId}:{ownerId:string}) {
  ...
  return (
    <div className='min-h-screen bg-zinc-50 text-zinc-500'>
        <div className='px-12 sticky top-0 z-40 bg-white border-b border-zinc-200'>
           <div className='max-w-7xl mx-auto px-6 h-16 flex items-center justify-between'>
             <div className='text-lg font-semibold cursor-pointer text-black' onClick={()=>navigate.push("/")}>Tynexa <span className='text-zinc-400'>AI</span></div>
             <button className='px-4 py-2 rounded-lg border cursor-pointer border-zinc-300 text-sm hover:bg-zinc-100 transition' onClick={()=>navigate.push("/dashboard")}>Back to Dashboard</button>
           </div>
        </div>
    ...
    <div className='flex justify-center px-4 py-5'>
        ...
        className='w-full max-w-4xl bg-white rounded-2xl shadow-xl p-14'
        >
        <h1 className='text-2xl font-semibold'>Embed Chatbot</h1>
        <p className='mb-4'>Copy and paste this code before <code>&lt;/body&gt;</code> </p>
        <div className='relative bg-zinc-900 text-zinc-100 rounded-xl p-5 text-sm font-mono mb-10'>
         ...
         <button className='absolute top-3 right-3 bg-white text-zinc-900 text-xs
                            font-medium px-3 py-1.5 rounded-lg hover:bg-zinc-200 transition cursor-pointer' onClick={copyCode}>
           {copied?"Copied ✔":"Copy"}
         </button>
        </div>
       ...
       <div className='mt-14'>
        <h1 className='text-lg font-medium '>Live Preview</h1>
        <p className='text-sm text-zinc-500 mb-5'>This is how the chatbot will appear on your website!</p>
        ...
        <div className='rounded-xl border border-zinc-300 bg-white shadow-md overflow-hidden'>
            ...
            <div className=' relative h-64 sm:h-72 p-6 text-zinc-400 text-sm'>Your website goes here 
              <div className='absolute bottom-16 right-6 w-64 bg-white rounded-xl shadow-xl border border-zinc-200 overflow-hidden mt-4'>
